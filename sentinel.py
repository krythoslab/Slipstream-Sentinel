...
class AutoModEngine:
    ...
    async def process_message(self, message: discord.Message):
        ...
        for link in links:
            ...
            elif re.search(r"(verify|login|account|steam|free.*nitro|prize|gift)", link, re.IGNORECASE):
...
async def status(interaction: discord.Interaction):
    uptime = utc_now() - client._start
    await interaction.response.send_message(
        f"Servers: {len(client.guilds)} | Uptime: {uptime} | Latency: {round(client.latency*1000)}ms | DB: {os.path.basename(DB_PATH)}"
    )
...
async def help(interaction: discord.Interaction):
    categories = {
        ...
        "Welcome/Leave": ["/welcome setup", "/welcome enable", "/welcome disable", "/welcome test"],
...
@client.tree.command(name="welcome", description="Welcome/leave configuration")
@app_commands.describe(action="setup, enable, disable, test")
...
async def warn(interaction: discord.Interaction, user: discord.Member, reason: str = "No reason provided."):
    ...
    if not role_hierarchy_ok(interaction.user, user):
        ...
        return
    case_id = get_next_case_id(interaction.guild_id)
    db_execute(
    ...
    log_mod_action(interaction.guild_id, "warn", user.id, interaction.user.id, reason)
    await interaction.response.send_message(f"Warned {user.mention} (Case #{case_id}): {reason}")
...
async def unwarn(interaction: discord.Interaction, warning_id: int):
    ...
    log_mod_action(interaction.guild_id, "unwarn", row["user_id"], interaction.user.id, f"Removed warning #{warning_id}")
...
async def timeout(interaction: discord.Interaction, user: discord.Member, minutes: int, reason: str = "No reason provided."):
    ...
    log_mod_action(interaction.guild_id, "timeout", user.id, interaction.user.id, reason, duration_seconds=minutes*60)
...
async def untimeout(interaction: discord.Interaction, user: discord.Member):
    ...
    log_mod_action(interaction.guild_id, "untimeout", user.id, interaction.user.id, "Timeout removed")
...
async def kick(interaction: discord.Interaction, user: discord.Member, reason: str = "No reason provided."):
    ...
    log_mod_action(interaction.guild_id, "kick", user.id, interaction.user.id, reason)
    await interaction.response.send_message(f"Kicked {user.mention}.")

@client.tree.command(name="ban", description="Ban a user")
@app_commands.describe(user="User ID or mention", reason="Reason", delete_message_days="Days of messages to delete")
async def ban(interaction: discord.Interaction, user: discord.User, reason: str = "No reason provided.", delete_message_days: int = 0):
    ...
    log_mod_action(interaction.guild_id, "ban", user.id, interaction.user.id, reason)
...
async def unban(interaction: discord.Interaction, user_id: str):
    ...
    try:
        ...
        log_mod_action(interaction.guild_id, "unban", user.id, interaction.user.id, "Unbanned")
...
async def purge(interaction: discord.Interaction, count: int):
    ...
    try:
        ...
        log_mod_action(interaction.guild_id, "purge", 0, interaction.user.id, f"Purged {len(deleted)} messages", channel_id=interaction.channel_id)
...
async def lock(interaction: discord.Interaction, channel: discord.TextChannel = None):
    ...
    try:
        ...
        log_mod_action(interaction.guild_id, "lock", 0, interaction.user.id, f"Locked {channel.name}", channel_id=channel.id)
...
async def unlock(interaction: discord.Interaction, channel: discord.TextChannel = None):
    ...
    try:
        ...
        log_mod_action(interaction.guild_id, "unlock", 0, interaction.user.id, f"Unlocked {channel.name}", channel_id=channel.id)
...
async def slowmode(interaction: discord.Interaction, channel: discord.TextChannel = None, seconds: int = 0):
    ...
    try:
        ...
        log_mod_action(interaction.guild_id, "slowmode", 0, interaction.user.id, f"Set slowmode {seconds}s", channel_id=channel.id)
...
async def history(interaction: discord.Interaction, user: discord.Member):
    ...
    if not rows:
        ...
        return
    lines = [f"#{r['id']} {r['action']} by <@{r['moderator_id']}>: {r['reason']} ({r['created_at'][:10]})" for r in rows]
    await interaction.response.send_message("\n".join(lines[:10]))
...
async def automod(interaction: discord.Interaction, action: str, user: discord.Member = None, keyword: str = None, threshold: int = None):
    ...
    if action == "status":
        ...
        if threshold is None:
            ...
            return
        # In a full implementation, store thresholds in DB; here we update runtime config
...
async def config(interaction: discord.Interaction, action: str, channel: discord.TextChannel = None, message: str = None, seconds: int = 0):
    ...
    if action == "welcome":
        ...
        if not channel:
            await interaction.response.send_message("Provide a channel.", ephemeral=True)
            return
        row = db_fetch("SELECT * FROM server_config WHERE guild_id = ?", (guild_id,), one=True)
        if not row:
            db_execute("INSERT INTO server_config (guild_id) VALUES (?)", (guild_id,))
        db_execute("UPDATE server_config SET modlog_channel_id = ? WHERE guild_id = ?", (channel.id, guild_id))
...
async def announce(interaction: discord.Interaction, channel: discord.TextChannel, message: str):
    if not permission_or(interaction, "manage_messages", "administrator"):
        await interaction.response.send_message("Missing permissions.", ephemeral=True)
        return
    embed = discord.Embed(title="Announcement", description=message, color=0x0099ff)
...
async def role(interaction: discord.Interaction, action: str, user: discord.Member, role: discord.Role):
    ...
    try:
        if action == "give":
            ...
            log_mod_action(interaction.guild_id, "role_add", user.id, interaction.user.id, f"Added role {role.name}")
            ...
            log_mod_action(interaction.guild_id, "role_remove", user.id, interaction.user.id, f"Removed role {role.name}")
...
@app_commands.describe(action="register, team, standings, season")
async def league(interaction: discord.Interaction, action: str, name: str = None, number: int = None, team_name: str = None):
    if action == "register":
        ...
        season_row = db_fetch("SELECT id FROM league_seasons WHERE guild_id = ? AND active = 1", (interaction.guild_id,), one=True)
        season_filter = f"AND r.race_id IN (SELECT id FROM league_races WHERE guild_id = {interaction.guild_id} AND season_id = {season_row['id']})" if season_row else ""
        rows = db_fetch(f"""
        ...
        db_execute(
            "INSERT INTO league_seasons (guild_id, name, created_at) VALUES (?, ?, ?)",
            (interaction.guild_id, name, utc_iso())
        )
        await interaction.response.send_message(f"Season '{name}' created.")
    else:
        await interaction.response.send_message("Unknown action.", ephemeral=True)
